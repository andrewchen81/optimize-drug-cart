...
def greedy_algorithm(drug_items, number_of_carts):
...
def best_fit_decreasing_algorithm(drug_items, number_of_carts):
    sorted_units = sorted(drug_items, key=lambda x: x[1], reverse=True)
    carts = [[] for _ in range(number_of_carts)]
    cart_weights = [0] * number_of_carts
...
def optimize_cart_allocation(
        ...
        drug_unit_col,
        drug_count_col,
        unit_constraint_dict,
        number_of_carts,
        algorithm
):
    # Prepare drug units and counts
    ...
    df_optimized_cart = pd.DataFrame(cart_data, index=["Drug Units", "Cart Total Weight"]).T.reset_index()
    df_optimized_cart = df_optimized_cart.rename(columns={'index': 'Cart Assignment'})
